  ...
  return (
    <div className="rounded-lg border border-neutral-800 bg-neutral-900/40 p-3 space-y-2">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <span className="text-sm font-medium text-neutral-200">{slate.name}</span>

        <div className="flex items-center gap-2">
          <button onClick={() => playSlate(slate.id)} className="px-2 py-1 text-xs rounded bg-neutral-800 border border-neutral-700">
            Play
          </button>

          {clipboard && (
            <button onClick={handlePasteHere} className="px-2 py-1 text-xs rounded bg-emerald-700 hover:bg-emerald-600">
              Paste at Playhead
            </button>
          )}
          ...
          <label className="flex items-center gap-1 text-xs text-neutral-500">
            ...
            <input
              ...
              className="w-16 px-1 py-0.5 rounded bg-neutral-950 border border-neutral-800 text-neutral-200"
            />
            s
          </label>
          ...
          <button
            ...
            className={`px-2 py-1 text-xs rounded border ${
              slate.muted ? "bg-red-900/40 border-red-800 text-red-300" : "bg-neutral-800 border-neutral-700"
            }`}
          >
          ...
          <button onClick={() => removeSlate(slate.id)} className="px-2 py-1 text-xs rounded bg-red-900/40 border border-red-800 text-red-300">
            Delete Slate
          </button>
        </div>
      </div>
      ...
      <div
        className="relative h-14 rounded bg-neutral-950 border border-neutral-800 overflow-hidden cursor-pointer"
        style={{ width }}
        onClick={e => {
      ...
      >
        {slate.regions.map(region => {
          ...
          return (
            ...
            >
              <span className="text-[10px] truncate text-indigo-100">
              ...
              <button
                ...
                className="ml-auto opacity-0 group-hover:opacity-100 text-[10px] text-red-300 px-1"
              >
                ✕
              </button>
            </div>
