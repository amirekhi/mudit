...
export default function ChartCarousel({ tracks, title }: Props) {
  ...
  return (
    <div className="relative w-full">
      ...
      <SectionHeader eyebrow="Charts" title={title} accent="discover" />
      ...
      >
        {tracks.map((track, i) => (
          <ChartTrackCard key={track._id} track={track} rank={i + 1} />
        ))}
      </div>
    </div>
  );
