...
import { IconMusic, IconAlertCircle } from "@tabler/icons-react";
...
export default function PublicSharedPlaylistPage() {
  ...
  useEffect(() => {
    fetch(`/api/share/public/${playlistId}`)
      .then(async (res) => {
        if (!res.ok) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 flex items-center justify-center text-neutral-500 dark:text-neutral-400 transition-colors">Loading…</div>
  );
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 px-4 py-10 transition-colors">
      <div className="max-w-2xl mx-auto">
        ...
        >
          {playlist?.image && (
            <img src={playlist.image} alt={playlist.title} className="w-full h-56 object-cover" />
          )}
            ...
            <div className="space-y-1 pt-2">
              {playlist?.tracks.map((track, i) => (
                <div
                  ...
                  className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 cursor-pointer transition-colors"
                >
